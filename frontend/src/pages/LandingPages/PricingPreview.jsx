...
const TiltCard = ({ children, style }) => {
    ...
    return (
        <motion.div ref={ref}
            ...
            onMouseLeave={isTouchDevice ? undefined : () => { rx.set(0); ry.set(0); }}
            style={{ ...style, rotateX: isTouchDevice ? 0 : rotateX, rotateY: isTouchDevice ? 0 : rotateY, transformStyle: 'preserve-3d' }}
        >
            {children}
        </motion.div>
    );
...
const cardStyle = {
    style: {
        base: {
            ...
            fontFamily: 'system-ui, sans-serif', fontSmoothing: 'antialiased',
...
const StripeCheckoutForm = ({ planName, clientSecret, priceLabel, onSuccess, onClose }) => {
    ...
    const handleSubmit = async () => {
        ...
        const { error: confirmError, paymentIntent } = await stripe.confirmCardPayment(
            clientSecret, { payment_method: { card: cardNumber } }
        );
        if (confirmError) {
            setError(confirmError.message); setLoading(false);
        } else if (paymentIntent?.status === 'succeeded') {
            setSucceeded(true);
        } else {
            setError('Payment could not be completed. Please try again.'); setLoading(false);
        }
    };
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, background: 'rgba(7,19,32,0.8)', border: '1px solid rgba(23,182,168,0.20)', borderRadius: 14, padding: 16 }}>
                <CardField label="Card Number"><CardNumberElement options={cardStyle} /></CardField>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    <CardField label="Expiration Date"><CardExpiryElement options={cardStyle} /></CardField>
                    <CardField label="Security Code"><CardCvcElement options={cardStyle} /></CardField>
                </div>
            </div>
            <AnimatePresence>
                {error && (
            ...
            <button onClick={handleSubmit} disabled={!stripe || !elements || loading}
                style={{ padding: '14px 0', borderRadius: 14, border: 'none', cursor: (!stripe || loading) ? 'not-allowed' : 'pointer', background: loading ? 'rgba(23,182,168,0.4)' : 'linear-gradient(135deg,#17B6A8,#0d9e92)', color: '#fff', fontWeight: 700, fontSize: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 8px 28px rgba(23,182,168,0.35)', transition: 'opacity 0.2s', opacity: loading ? 0.7 : 1, fontFamily: 'system-ui, sans-serif' }}>
                {loading ? (
                    <><span style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%', display: 'inline-block', animation: 'spinLoader 0.7s linear infinite' }} />Processing...</>
                ) : (
                    <><Lock style={{ width: 14, height: 14 }} />Pay {priceLabel} Securely</>
                )}
            </button>
...
const PaymentModal = ({ plan, billingCycle, onClose, onSuccess }) => {
    ...
    const fetchIntent = useCallback(async () => {
        if (isFree) return;
        setFetching(true); setFetchError(null); setClientSecret(null);
        try {
            const res = await fetch(`${BACKEND_URL}/api/create-payment-intent`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ amount: amountCents, currency: 'usd', planName: plan.name, billingCycle }),
            });
            if (!res.ok) { const text = await res.text().catch(() => ''); throw new Error(`Server responded with ${res.status}${text ? `: ${text}` : ''}`); }
    ...
    const appearance = {
        theme: 'night',
        variables: { colorPrimary: '#17B6A8', colorBackground: '#071320', colorText: '#f1f5f9', colorDanger: '#f87171', fontFamily: 'system-ui, sans-serif', borderRadius: '10px', spacingUnit: '4px' },
        rules: {
    ...
    return (
        <div
            style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
            onClick={onClose}
        >
            <motion.div
            ...
            >
                {/* Modal Header */}
                <div style={{ background: `linear-gradient(135deg, ${plan.accent}25, ${plan.accent}08)`, borderBottom: `1px solid ${plan.border}`, padding: '20px 22px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', position: 'sticky', top: 0, zIndex: 10, backdropFilter: 'blur(12px)' }}>
                    <div>
                        <p style={{ color: 'rgba(255,255,255,0.45)', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 4px', fontFamily: 'system-ui, sans-serif' }}>Secure Checkout · Stripe</p>
                        <h3 style={{ color: '#fff', fontWeight: 800, fontSize: 17, margin: '0 0 12px', fontFamily: 'system-ui, sans-serif' }}>{plan.name} Plan</h3>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                            <span style={{ color: plan.accent, fontWeight: 900, fontSize: 32, fontFamily: 'system-ui, sans-serif' }}>{priceLabel}</span>
                ...
                <div style={{ padding: '20px 22px' }}>
                    {fetchError ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                            <div style={{ background: 'rgba(239,68,68,0.10)', border: '1px solid rgba(239,68,68,0.30)', borderRadius: 12, padding: '14px 16px', color: '#F87171', fontSize: 13, fontFamily: 'system-ui, sans-serif', lineHeight: 1.5 }}>⚠️ {fetchError}</div>
                            <button onClick={fetchIntent} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '12px 0', borderRadius: 12, background: 'linear-gradient(135deg,#17B6A8,#0d9e92)', border: 'none', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: 'system-ui, sans-serif' }}>
                                <RefreshCw style={{ width: 14, height: 14 }} /> Retry
                            </button>
                            <button onClick={onClose} style={{ width: '100%', padding: '12px 0', borderRadius: 12, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.10)', color: '#fff', fontWeight: 600, fontSize: 13, cursor: 'pointer', fontFamily: 'system-ui, sans-serif' }}>Close</button>
                        </div>
                    ) : isFree ? (
                        ...
                        <Elements key={clientSecret} stripe={getStripePromise()} options={{ clientSecret, appearance, locale: 'en' }}>
                            <StripeCheckoutForm planName={plan.name} clientSecret={clientSecret} priceLabel={priceLabel} onSuccess={onSuccess} onClose={onClose} />
                        </Elements>
                    )}
                </div>
            </motion.div>
    ...
    const plans = [
        {
            ...
            accent: '#94A3B8', glow: 'rgba(148,163,184,0.18)', border: 'rgba(148,163,184,0.25)',
            ...
            btn: 'Start Free', btnBg: 'rgba(148,163,184,0.12)', btnColor: '#CBD5E1', btnBorder: '1px solid rgba(148,163,184,0.28)',
            popular: false, delay: 0.1,
        },
        {
            name: 'Pro',
            ...
            accent: '#17B6A8', glow: 'rgba(23,182,168,0.28)', border: 'rgba(23,182,168,0.55)',
            ...
            btn: 'Get Started', btnBg: 'linear-gradient(135deg, #17B6A8, #0d9e92)', btnColor: '#fff', btnShadow: '0 8px 28px rgba(23,182,168,0.45)',
        ...
        },
        {
            ...
            accent: '#F5C842', glow: 'rgba(245,200,66,0.18)', border: 'rgba(245,200,66,0.28)',
            ...
            btn: 'Contact Sales', btnBg: 'rgba(245,200,66,0.10)', btnColor: '#F5C842', btnBorder: '1px solid rgba(245,200,66,0.30)',
            popular: false, delay: 0.3,
        },
    ];

    return (
        <section style={{ position: 'relative', padding: 'clamp(60px, 8vw, 90px) clamp(16px, 4vw, 24px) clamp(60px, 8vw, 100px)', overflow: 'hidden', fontFamily: 'system-ui, sans-serif', background: 'linear-gradient(150deg, #071320 0%, #0b1d2e 55%, #060f1a 100%)' }}>
            <style>{`
                ...
                @media (max-width: 380px) {
                    .billing-btn { padding: 9px 16px; font-size: 13px !important; }
                }
                ...
                @media (max-width: 480px) {
                    .price-number { font-size: 38px !important; }
                }
                ...
                @media (max-width: 480px) {
                    .plan-card-inner { padding: 24px 20px !important; }
                }
            `}</style>
            ...
            {/* Blobs */}
            <div style={{ position: 'absolute', top: '-10%', left: '-10%', zIndex: 1, width: 'clamp(300px, 50vw, 700px)', height: 'clamp(300px, 50vw, 700px)', borderRadius: '50%', pointerEvents: 'none', background: 'radial-gradient(circle, rgba(23,182,168,0.15) 0%, transparent 65%)', animation: 'blobBreath1 8s ease-in-out infinite' }} />
            <div style={{ position: 'absolute', bottom: '-8%', right: '-8%', zIndex: 1, width: 'clamp(250px, 45vw, 600px)', height: 'clamp(250px, 45vw, 600px)', borderRadius: '50%', pointerEvents: 'none', background: 'radial-gradient(circle, rgba(245,200,66,0.07) 0%, transparent 65%)', animation: 'blobBreath2 10s ease-in-out 2.5s infinite' }} />
            {PARTICLES.map((p, i) => <Particle key={i} {...p} />)}

            <div style={{ maxWidth: 1180, margin: '0 auto', position: 'relative', zIndex: 5 }}>

                {/* ── HEADER ── */}
                <motion.div initial={{ opacity: 0, y: 36 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.75, ease: [0.22, 1, 0.36, 1] }} style={{ textAlign: 'center', marginBottom: 'clamp(40px, 6vw, 64px)' }}>
                    <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginBottom: 20 }}>
                        <div style={{ width: 28, height: 2, background: '#17B6A8' }} />
                        <span style={{ color: '#17B6A8', fontSize: 'clamp(10px, 2vw, 11px)', fontWeight: 700, letterSpacing: '3px', textTransform: 'uppercase', fontFamily: 'system-ui, sans-serif' }}>Simple &amp; Transparent Pricing</span>
                        <div style={{ width: 28, height: 2, background: '#17B6A8' }} />
                    </motion.div>

                    <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }}
                        style={{ fontSize: 'clamp(28px, 5vw, 54px)', fontWeight: 800, letterSpacing: '-1px', lineHeight: 1.1, color: '#fff', fontFamily: 'system-ui, sans-serif', margin: '0 0 32px' }}>
                    ...
                    <motion.div initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.35 }}
                        ...
                        {['monthly', 'yearly'].map(cycle => (
                            <motion.button key={cycle} onClick={() => setBillingCycle(cycle)} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.95 }}
                                ...
                                style={{ borderRadius: 12, border: 'none', cursor: 'pointer', fontFamily: 'system-ui, sans-serif', fontSize: 14, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, transition: 'background 0.3s, color 0.3s, box-shadow 0.3s',
                    ...
                    {plans.map((plan, index) => {
                        ...
                        return (
                            <motion.div key={index}
                                ...
                                viewport={{ once: true }}
                                transition={{ duration: 0.7, delay: plan.delay, ease: [0.22, 1, 0.36, 1] }}
                                ...
                                <TiltCard style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                    <motion.div
                                        ...
                                        style={{ flex: 1, display: 'flex', flexDirection: 'column', borderRadius: 26, background: plan.bg, border: `1px solid ${plan.border}`, backdropFilter: 'blur(24px)', WebkitBackdropFilter: 'blur(24px)', position: 'relative', overflow: 'hidden' }}>
                                        ...
                                        <div style={{ marginBottom: 22 }}>
                                            <h3 style={{ color: '#fff', fontSize: 'clamp(18px, 2.5vw, 22px)', fontWeight: 800, margin: '0 0 10px', fontFamily: 'system-ui, sans-serif' }}>{plan.name}</h3>
                                            <div style={{ display: 'flex', alignItems: 'baseline', gap: 5, marginBottom: 6 }}>
                                                <AnimatePresence mode="wait">
                                                    <motion.span key={billingCycle + plan.name}
                                                        initial={{ opacity: 0, y: -20, scale: 0.8 }}
                                                        animate={{ opacity: 1, y: 0, scale: 1 }}
                                                        exit={{ opacity: 0, y: 20, scale: 0.8 }}
                                                        transition={{ duration: 0.32, ease: [0.22, 1, 0.36, 1] }}
                                                        className="price-number"
                                                        style={{ fontWeight: 900, lineHeight: 1, color: plan.accent, display: 'block', fontFamily: 'system-ui, sans-serif' }}>
                                        ...
                                        <div style={{ marginTop: 'auto' }}>
                                            <motion.button
                                                ...
                                                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', padding: '14px 0', borderRadius: 14, cursor: 'pointer', fontFamily: 'system-ui, sans-serif', fontSize: 14, fontWeight: 700, letterSpacing: '0.03em',
                                                    border: isSubscribed ? '1px solid rgba(23,182,168,0.28)' : (plan.btnBorder || 'none'),
                ...
                <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.45 }}
                    className="trust-row"
                    style={{ marginTop: 'clamp(40px, 6vw, 64px)', paddingTop: 'clamp(24px, 4vw, 42px)', borderTop: '1px solid rgba(23,182,168,0.12)' }}>
                    {[
