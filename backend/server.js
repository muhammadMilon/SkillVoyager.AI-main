...
app.use(cors({
  origin: [
    'http://localhost:5173',
    'http://localhost:3000',
    'https://skill-voyager-ai.vercel.app',
  ],
...
// ✅ FIX: success_url এ courseId pass করা হচ্ছে
//    → Payment শেষে frontend /courses?purchased=COURSE_ID তে ফিরবে
//    → CoursesPage.jsx সেই courseId পড়ে localStorage এ save করবে (auto-unlock)
// ─────────────────────────────────────────────────────────────────────────────
app.post('/api/create-course-checkout-session', async (req, res) => {
  try {
    const { courseId, courseTitle, amount, currency, userEmail, successUrl } = req.body;

    if (!amount || !currency) {
      return res.status(400).json({ error: 'amount এবং currency দরকার' });
    }

    const clientUrl = process.env.CLIENT_URL || 'https://skill-voyager-ai.vercel.app';

    // ✅ successUrl: frontend থেকে পাঠানো URL ব্যবহার করো (courseId সহ),
    //    না পাঠালে fallback হিসেবে নিজে বানাও
    const finalSuccessUrl =
      successUrl ||
      `${clientUrl}/courses?purchased=${String(courseId || '')}&session_id={CHECKOUT_SESSION_ID}`;
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // ✅ এখন success এ /courses?purchased=COURSE_ID তে যাবে
...
// ── Course Payment Success (Stripe redirect থেকে ফিরে enrollment save) ───────
// এই endpoint টা frontend থেকে call করা হয় session verify করার জন্য
// GET /api/course-payment-success?session_id=...&courseId=...
// ✅ Frontend এ CoursesPage.jsx URL থেকে courseId পড়ে localStorage এ save করে,
//    তারপর এই endpoint call করে DB তে enrollment নিশ্চিত করে
// ─────────────────────────────────────────────────────────────────────────────
app.get('/api/course-payment-success', async (req, res) => {
  try {
...
// ── পুরনো PATCH endpoint — backward compatibility এর জন্য রাখা হলো ──────────
app.patch('/api/course-payment-success', async (req, res) => {
  try {
    ...
    await Enrollment.create(enrollmentData);
    console.log('✓ Course enrollment saved (PATCH):', enrollmentData.courseTitle);
...
// ── Verify enrollment (frontend থেকে check করার জন্য) ───────────────────────
// GET /api/enrollment/verify?courseId=...&userEmail=...
app.get('/api/enrollment/verify', async (req, res) => {
  try {
    const { courseId, userEmail } = req.query;

    if (!courseId) {
      return res.status(400).json({ enrolled: false, message: 'courseId required' });
    }

    const query = { courseId: String(courseId), paymentStatus: 'paid' };
    ...
    console.error('Enrollment verify error:', error);
    res.status(500).json({ enrolled: false, message: 'Verification failed' });
  }
});
...
// ═══════════════════════════════════════════════════════════════════════════
// server.js এ এই পুরো block টা paste করুন
// যেকোনো existing app.get/app.post এর পরে, mongoose.connect এর আগে
// তারপর deploy করুন
// ═══════════════════════════════════════════════════════════════════════════
...
app.get('/api/verify-session', async (req, res) => {
  try {
    ...
    console.log('verify-session:', { courseId, courseTitle, status: session.payment_status });

    // DB তে save করো (duplicate check সহ)
    if (courseId && session.payment_status === 'paid') {
      const existing = await Enrollment.findOne({ transactionId: session.payment_intent }).catch(() => null);
      if (!existing) {
    ...
    res.json({ success: true, courseId, courseTitle });
